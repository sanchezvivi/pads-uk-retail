...
- revisar a descritiva geral (**Renato**)
  - limpar outliers (**Renato**)
  - escolher grupo (tea) e analisar mais a fundo
- revisar o PCA (**Vivi**)
- revisar o PCA com Reino Unido e Cores (**Vivi**)
- revisar a descritiva por categoria (**Vivi**)
- analise textual (https://www.tidytextmining.com/nasa.html) (**Débora**)
- Analisar a probabilidade de compra de um produto/categoria da missão (Logit) (quarta) (**Débora**)
- modelo de associação (arules)
  - normalizar variáveis (remover cores) (**Débora**)
- shinny (discutimos quarta) (**Vivi**)
...
```{r}
retail_word <- retail %>% select(Description)
retail_word <- str_c(retail_word, collapse="|")

  retail_word <- 
  data.frame(text = retail_word,stringsAsFactors = F) %>% 
  unnest_tokens(word, text) %>% plyr::count()

freq_by_rank <- retail_word %>% dplyr::arrange(desc(freq)) 
freq_by_rank
...
material <- str_c(str_to_upper(c('metal','wooden', 'wood', 'glass', 'silver', 'ceramic', 'acrylic','amber','amethyst', 'cooper', 'gold',  'zinc', 'tin',
'wire','crystal','diamante','iron' )), collapse="|")
stopword <- str_c(str_to_upper(c('set', 'with', 'colour')), collapse="|")

retail_2 <- 
  retail %>% 
  mutate(Description_2 = str_replace_all(Description,size,"")) %>% 
  mutate(Description_2 = str_replace_all(Description_2,material,"")) %>% 
  mutate(Description_2 = str_replace_all(Description_2,stopword,"")) %>% 
  mutate(Description_2 = str_replace_all(Description_2,colours,"")) %>%
  mutate(Description_2 = str_replace_all(Description_2,numbers,"")) %>% 
  mutate(Description_2 = str_replace_all(Description_2, "\\W", " ")) %>% 
  mutate(Description_2 = str_replace_all(Description_2, "\\s+$", "")) %>%
  mutate(Description_2 = str_replace_all(Description_2, "^\\s+", "")) %>%
  mutate(Description_2 = str_replace_all(Description_2, "\\s+", " "))
...
retail_2 <- 
  retail %>% 
    mutate(cores = str_extract(Description, colours)) %>% 
    mutate(material = str_extract(Description, material)) %>% 
    mutate(size = str_extract(Description, size))
...
retail_2 <- 
  retail %>% 
    mutate(Description_2 = str_replace_all(Description, c('ANT' = 'ANTIQUE','TURQ = TURQUESA','ASS = ASSORTED','ASSTD = ASSORTED','CUPCAKE' = 'CUP CAKES','CUPCAKES' = 'CUP CAKES','DRAWERKNOB' = 'DRAWER KNOB','ED' = 'EDWARDIAN'))) 
```
# PRODUTOS DISTINTOS
```{r}
retail %>% distinct(Description) %>% count()
retail_2 %>% distinct(Description_2) %>% count()
