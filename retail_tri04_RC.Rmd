...
retail %>% dplyr::group_by(NPS) %>% tally
...
```{r}
foras2 <- clientes %>% dplyr::filter(Tkt > 100) %>% select(CustomerID) 
...
df1 <- df %>%    dplyr::mutate(nome = rownames(df)) %>%   dplyr:: filter(PC1^2 > mean(PC1^2)) %>%    dplyr::select(c(PC1,nome))
#COMIDA E LANCHES
