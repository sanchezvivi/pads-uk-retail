...
#Dataframe agrupado por Ticketb
tickets <- retail %>% 
...
median(clientes$Tkt)
 ...
 ggplot(clientes, aes(y=Tkt)) + 
  ...
  ggplot(clientes, aes(y=Tkt)) +  coord_cartesian(ylim = c(0, 50))+
  geom_boxplot()
  
 # retail2 <- retail %>% dplyr::group_by(CustomerID,InvoiceNo) %>% 
    #dplyr::mutate(Ticket = length(unique(InvoiceNo))) %>% 
  #  dplyr::mutate(n = count(CustomerID,InvoiceNo)) %>% 
  #  dplyr::ungroup()

  #retail2 <- retail %>% dplyr::mutate(n = count(CustomerID,InvoiceNo))
  
  
```


#acima de 100 compras
# Analise de evolução
