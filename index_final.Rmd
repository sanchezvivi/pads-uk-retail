...
#cria markdown com variáveis do global env para carregar mais rápido
#rmarkdown::render("index.Rmd", envir=.GlobalEnv)
...
size <- str_c(str_to_upper(c('small', 'large', 'jumbo', 'box', 'pack',
                             'cases', 'kit', ' set')), collapse="|")

material <- str_c(str_to_upper(c('metal','wooden', 'wood', 'glass',
                                 'silver', 'ceramic', 'acrylic','amber',
                                 'amethyst', 'cooper', 'gold',  'zinc', 'tin',
                                 'wire','crystal','diamante','iron')), collapse="|")

#stopword <- str_c(str_to_upper(c(' with',' colour')), collapse="|")

filters <- str_c(c(size, material, colours, numbers), collapse = "|")
...
rt_uk <- rt_uk_prep %>% 
  mutate(product = str_replace_all(description, filters,""),
         ...
         product = str_replace_all(product, "^\\s+", ""),
         product = str_replace_all(product, "\\s+", " "),
         colour = str_extract(description, colours),
         made_from = str_extract(description, material),
         package = str_extract(description, size))
  
#rt_uk %>% fill("none") %>% skim()
...
cols_prod <- tibble(col = c("unit_price","tkts" ,"clients",
...
# itemList <- plyr::ddply(rt_uk,c("customer_id","date"), 
...
rules <- apriori(tr, parameter = list(sup = 0.004, conf = 0.8))
#inspect(head(rules, n = 10, by = c("confidence", "lift")))

#Filtrar as regras da missao de compra
rules.sub <- subset(rules, subset = rhs %in% products & lift > 2 )

#direita_raw <- rules.sub@rhs@itemInfo
#esquerda_raw <- rules.sub@lhs@itemInfo

Regency <-  rules.sub@rhs@itemInfo %>% filter(grepl("REGENCY",labels))
rules_regency <- subset(rules.sub , subset = rhs %in% Regency$labels )

lunch <-  rules.sub@rhs@itemInfo %>% filter(grepl("LUNCH",labels))
rules_lunch <- subset(rules.sub , subset = rhs %in% lunch$labels )

bags <-  rules.sub@rhs@itemInfo %>% filter(!grepl(paste(c("REGENCY","LUNCH"),collapse="|"),labels))
rules_bags <- subset(rules.sub , subset = rhs %in% bags$labels )

# Recomendação --------------

#source("logit_products.R")
...
#output$sales_line <- renderHighchart({
  ...
  rt_summary %>%  
    ...
    pivot_longer(cols = starts_with("perc"),
                 ...
                 names_to = "variable",
                 values_to = "pct") %>% 
    mutate(variable = if_else(variable == "net","Faturamento", 
                              "Transações")) %>% 
    hchart(., "column", 
...
- Para a análise decidimos tirar os casos mais atípicos, traçando um corte em de clientes até 100 pedidos.

```{r}

plotOutput("hist_clients")

```

```{r warning=FALSE, context="server"}

output$hist_clients <- renderPlot({
  
  clicks <- input$update_button
  
    isolate({

ggplot(clientes, aes(x=Tkt)) + 
  geom_histogram(binwidth=1) + 
...
x <- c("Receita Total (MM)", "Itens", "Preço Unitário")
y <- c("{point.total_revenue}", "{point.quantity}", "{point.unit_price}%")
...
selectInput("prod_id", label = "Selecione o produto", choices = append("ALL", products),
            multiple = T,
            selected = "ALL")
...
isolate({  
 ...
 if("ALL" %in% input$prod_id){products}else{input$prod_id}
...
rt_mission %>% 
  ...
  filter(description %in% selected_products()) %>%
