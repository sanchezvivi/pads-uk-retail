...
selectInput("var_id_2", label = "Variável", choices = cols_prod$name,
            multiple = F,
            selected = cols_prod$name[1])

selectInput("prod_id", label = "Produto", choices = append("ALL", products),
            multiple = T,
            selected = "ALL")
...
rt_days() %>% 
  filter(holiday == 0) %>% 
  ggplot(aes(x = dow, y = full_hour, fill = col)) +
  geom_tile() +
...
rt_days() %>% 
  filter(holiday == 1) %>% 
  ggplot(aes(x = dow, y = full_hour, fill = selected_col_2())) +
  geom_tile() +
  #scale_fill_viridis_c(labels = scales::label_number_si()) +
